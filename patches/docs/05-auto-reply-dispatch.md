...
| `src/auto-reply/reply.block-streaming.test.ts` | 流式阻塞回复测试 |
| `src/auto-reply/reply/abort.test.ts` | abort / force-stop 测试 |
| `src/auto-reply/reply/agent-runner.runreplyagent.e2e.test.ts` | `runReplyAgent` 端到端测试 |
| `src/auto-reply/reply/session-hooks-context.test.ts` | session hooks 上下文测试 |
| `src/auto-reply/reply/session.test.ts` | session 创建与保留测试 |
| `src/auto-reply/status.test.ts` | status 输出渲染测试 |
