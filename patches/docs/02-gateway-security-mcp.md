...
| `src/gateway/node-connect-reconcile.ts` | 调用 `resolveNodeCommandAllowlist` 时传入 `nodeId` 和 `displayName` 参数，适配新签名 |
| `src/gateway/server-methods/chat.ts` | 对齐 chat 方法中的节点命令策略调用 |
| `src/gateway/server-methods/nodes.ts` | 调用 `resolveNodeCommandAllowlist` 时传入节点标识参数，适配新签名 |
| `src/gateway/session-transcript-files.fs.ts` | 转录文件路径解析辅助，供 `session-archive.fs.ts` 归档逻辑使用 |
