...
| `src/agents/bootstrap-compaction.ts` | **新文件 (331行)**：LLM 驱动的 bootstrap 文件压缩，含内容哈希缓存、head/tail 截断、超时控制 |
...
| `src/agents/bash-tools.exec.pty-fallback-failure.test.ts` | bash 工具 PTY 回退失败场景测试 |
| `src/agents/bash-tools.process.supervisor.test.ts` | bash 工具进程 supervisor 测试 |
| `src/agents/models-config.providers.openai-codex.test.ts` | OpenAI Codex 提供商配置测试 |
| `src/agents/openclaw-tools.web-runtime.test.ts` | OpenClaw 工具 web 运行时测试 |
| `src/agents/pi-embedded-helpers.isbillingerrormessage.test.ts` | `isBillingErrorMessage` 辅助函数测试 |
| `src/agents/pi-embedded-runner/extra-params.openai.test.ts` | OpenAI extra params 测试 |
| `src/agents/pi-embedded-runner/extra-params.xai-tool-payload.test.ts` | xAI tool payload extra params 测试 |
| `src/agents/skills/skill-contract.ts` | skill 契约定义 |
| `src/agents/tool-summaries.ts` | 工具摘要生成 |
