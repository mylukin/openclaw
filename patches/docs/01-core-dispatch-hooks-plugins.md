...
| `src/channels/plugins/setup-wizard-helpers.test.ts` | 频道插件引导向导辅助函数的单元测试 |
...
| `src/config/zod-schema.agent-runtime.ts` | Agent runtime schema 新增字段同步 |
| `src/config/zod-schema.ts` | 顶层 Zod schema barrel 更新，导出新增的 override / agent-defaults schema |
| `src/config/sessions/artifacts.ts` | 会话制品（artifacts）类型与持久化辅助 |
| `src/config/sessions/disk-budget.ts` | 会话磁盘预算计算逻辑扩展 |
| `src/config/sessions/disk-budget.test.ts` | 磁盘预算计算的单元测试 |
| `src/config/sessions/sessions.test.ts` | 会话管理核心逻辑的单元测试 |
| `src/config/config-misc.test.ts` | 杂项配置辅助函数的单元测试 |
| `src/config/config.legacy-config-detection.rejects-routing-allowfrom.test.ts` | 旧配置检测拒绝 `routing.allowFrom` 的回归测试 |
| `src/plugins/manifest-registry.ts` | 注册表构建时保留 hook 元数据（`model`、`timeout` 等） |
| `src/infra/outbound/current-conversation-bindings.ts` | 当前会话绑定辅助：conversation ID 绑定更新 |
...
| `src/security/audit-extra.sync.ts` | 安全审计扩展同步检查逻辑更新 |
| `src/security/audit.test.ts` | 安全审计逻辑的单元测试 |
